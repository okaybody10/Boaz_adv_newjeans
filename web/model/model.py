...
def preprocess_data(text, image):
    ...
    text = text_embedder.encode(text, convert_to_tensor=True).unsqueeze(0)

    image = Image.open(image).convert("RGB")
    image_transform = JointTextImageModel.build_image_transform()
    image = image_transform(image).unsqueeze(0)
    label = torch.tensor([0]).squeeze().unsqueeze(0)

    return [text, image, label]

# # load .ckpt file
# ckpt = "D:\\분석adv\\lightning_logs\\version_5\\checkpoints\\epoch=9-step=1000.ckpt"
# model = MultimodalFakeNewsDetectionModel.load_from_checkpoint(ckpt, map_location='cpu')

# with torch.no_grad():
#     model.eval()
#     input_text = "한국 건강식품·화장품 기업 대상 ‘2017 홍콩바이어 매칭 박람회’ 열린다"
#     input_img = "D:\\분석adv\\web\\test.jpg"
#     text_embedder= SentenceTransformer('snunlp/KR-SBERT-V40K-klueNLI-augSTS')
#     image_transform = JointTextImageModel.build_image_transform()
    
#     text_tensor, image_tensor, label_tensor = preprocess_data(input_text, input_img)
#     #print(text_tensor.shape, image_tensor.shape, label_tensor.shape)
#     prediction, loss = model(text_tensor, image_tensor, label_tensor)
#     pred_label = torch.argmax(prediction, dim=1)
#     print("input text : ", input_text)
#     print("진짜 뉴스라고 예측한 값 : ", prediction[0][0].item())
#     print("가짜 뉴스라고 예측한 값 : ", prediction[0][1].item())
#     print("loss : ", loss.item())
#     print("예측 결과 : ", pred_label.item())
