...
if __name__ == "__main__":
    ...
    config = {}
    if args.config != "":
        with open("./configs/" + str(args.config), "r") as yaml_file:
            config = yaml.full_load(yaml_file)

    args.train_data_path = config.get("train_data_path", os.path.join(DATA_PATH, "multimodal_train_" + str(TRAIN_DATA_SIZE) + ".csv"))
    args.test_data_path = config.get("test_data_path", os.path.join(DATA_PATH, "multimodal_test_" + str(TEST_DATA_SIZE) + ".csv"))
